# DevSecOps Assessment — Track 1 Documentation

## Executive Summary

### Overall Risk Posture: High — Remediable Before Go-Live

This submission presents a comprehensive security review and target-state design was conducted against a kubernetes-goat reference target.

#### Key Findings: 28 security findings identified — **6 Critical, 12 High, 7 Medium, 1 Low**.  finding (K8S-001) mirrors the trigger incident: a `ClusterRoleBinding` granting `cluster-admin` to a CI job's ServiceAccount, enabling full cluster compromise from any workload.

### What's Fixable Before Migration (Next Quarter):
...
### Target-state Architecture: Defence-in-depth across four independent layers:

1. **Edge**: WAF + ALB
2. **Admission**: Kyverno
3. **Network**: Cilium NetworkPolicy + WireGuard mTLS  
4. **Runtime**: Falco 0.38.x detection

Supply chain secured via:
- Conftest OPA policies
- Trivy scanning  
- Cosign image signing
- 6-stage GitLab CI/CD pipeline with hard-fail gates on Critical/High findings
- Secrets managed via External Secrets Operator + cloud-native secrets managers, portable across AWS/Azure/GCP

### Recommendation: Approve the 30/60/90-day implementation plan and COTS risk acceptance to proceed with the migration wave.

## Programme Context

Multi-cloud government programme running ~100 workloads on AWS (Terraform + GitLab CI/CD), split across two EKS landing zone tiers. Azure and GCP onboarding on the roadmap within two quarters. ~18 workloads on EKS run citizen-facing services with unpredictable traffic.
